...
* Make TSV button smaller on PDA, similar size/style to the FF button. `{v8.3.0}`
* Disable TSV entirely on racing page.`{v8.3.0}`
...
* When editing an existing custom rule, auto-populate the saved values instead of opening blank/default fields.  `{v8.3.0}`
* Allow viewing, updating, and deleting existing rule configuration directly from the item.`{v8.3.0 partial}`
* Make it visually clear when an item already has a saved custom rule.`{v8.3.0 partial}`
...
* Existing custom rule editing partially implemented but needs fixing because saved rule data and editor-loading format do not fully line up. `{v8.3.0}`
